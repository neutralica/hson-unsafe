...
function normalize_style(s: string | Record<string, string> | undefined): Record<string, string> | undefined {
  if (!s) return undefined;
  if (typeof s === "string") {
    /* naive css parser: split on ';' then ':'; ignores dupes/!important; good enough for parity */
    const out: Record<string, string> = {};
    for (const decl of s.split(";")) {
      const [k, v] = decl.split(":").map(t => t?.trim()).filter(Boolean) as [string?, string?];
      if (k) out[k] = v ?? "";
    }
    return out;
  }
  /* already object; clone shallow */
  return { ...s };
}
...
function normalize_attrs(attrs: HsonAttrs | undefined): Record<string, Primitive> {
  const a = { ...(attrs ?? {}) } as Record<string, Primitive>;
  if ("style" in a) {
    const norm = normalize_style(a["style"] as any);
    if (norm) a["style"] = JSON.parse(JSON.stringify(norm)) as any;
  }
  return a;
}
...
function normalize_meta(meta: HsonMeta | undefined) {
  return {
    attrs: normalize_attrs(meta?.attrs),
    flags: normalize_flags(meta?.flags),
    /* include known scalar meta fields if present */
...
function normalize_node(n: HsonNode | Primitive): any {
  ...
  return { _tag: tag, _meta: meta, _content: content };
}

/* equality: structural, order-insensitive for attrs/flags */
export function equal_old_nodes(a: HsonNode, b: HsonNode): boolean {
  return stable_stringify(normalize_node(a)) === stable_stringify(normalize_node(b));
}
