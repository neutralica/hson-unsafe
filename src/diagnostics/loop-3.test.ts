 ...
 *   JSON ↔ HTML ↔ HSON
 *
 * The core goal is to detect structural drift by repeatedly:
 ...
 *   - regression testing
 *   - fixture validation
 *   - debugging subtle serializer / parser mismatches
 *
 * It is safe to ship and inexpensive to run.
 */
...
/**
 * Runs a multi-cycle round-trip validation across JSON, HTML, and HSON.
 *
 * Given an input fixture (string, object, DOM node, or HsonNode),
 * this function:
 *
 *   • resolves an entry representation (explicit or auto-detected),
 *   • converts it into a canonical HsonNode,
 *   • walks a fixed “ring” of formats (cw or ccw),
 *   • re-parses on each step,
 *   • asserts structural invariants after every parse,
 *   • diffs consecutive nodes for semantic equivalence,
 *   • optionally repeats the ring multiple times,
 *   • returns a structured report describing success or failure.
 *
 * The function does **not** mutate global state and does **not** log.
 * All diagnostics are returned in the LoopReport object.
 *
 * @param atom
 *   The input fixture to validate.
 *   Accepted forms:
 ...
 *
 * @param opts.times
 *   Number of full ring cycles to run.
 *   Defaults to 3.
 *
 * @param opts.dir
 ...
 *
 * @param opts.entry
 *   Explicit entry format.
 *   `"auto"` attempts json → html → hson via try-parse.
 *   Defaults to `"auto"`.
 *
 * @param opts.verbose
 *   When true, includes a full step-by-step trace in the report.
 *   Defaults to false.
 *
 * @param opts.stopOnFirstFail
 *   When true, aborts immediately on the first failed comparison.
 *   Defaults to true.
 *
 * @returns
 *   A LoopReport describing:
 *     - overall success/failure,
 *     - which step(s) failed (if any),
 *     - optional trace data,
 *     - and the final serialized representation.
 *
 * @example
 * _test_full_loop(`<div><span>Hello</span></div>`);
 *
 * @example
 * _test_full_loop(myJsonFixture, { times: 5, verbose: true });
 *
 * @example
 * _test_full_loop(hsonNode, { entry: "node", dir: "ccw" });
 */
...
function is_html_element(x: unknown): x is HTMLElement {
    // avoid relying on HTMLElement existing in Node unless provided by happy-dom
