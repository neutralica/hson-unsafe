...
import { is_Primitive, is_void } from "../../../core/utils/guards.core.utils.hson";
import { ELEM_TAG, OBJ_TAG, _FALSE, STR_TAG, VAL_TAG, II_TAG, ARR_TAG } from "../../../types-consts/constants.hson";
import { HsonMeta, HsonNode } from "../../../types-consts/node.types.hson";
import { format_hson_attrs } from "../../../utils/format-hson-attrs.utils.hson";
import { get_self_close_value } from "../../../utils/get-self-value.utils.hson";
...
function formatHsonMeta($meta: HsonMeta | undefined): string {
    ...
    /* only include attrs if there are any */
    const hasAttrs = !!($meta.attrs && Object.keys($meta.attrs).length);
    const attributesString = hasAttrs ? format_hson_attrs($meta.attrs) : "";

    /* CHANGED: only include flags when length > 0 (empty array is truthy!) */
    const flagsString = (Array.isArray($meta.flags) && $meta.flags.length > 0)
        ? ` ${$meta.flags.join(' ')}`
        : "";
...
function hsonFromNode(
    ...
    if (tag === STR_TAG || tag === VAL_TAG) {
        if (content.length !== 1 || !is_Primitive(content[0])) {
            _throw_transform_err('_str or _val nodes must have 1 and only 1 hson  in content', 'serialize-hson');
        }
    ...
    if (tag === II_TAG) {
        if (content.length !== 1) { _throw_transform_err('_ii nodes must have 1 & only 1 child', 'serialize-hson'); }
        if (!is_Node(content[0])) {
            _throw_transform_err('index tag contents are not nodes', 'serialize-hson');
        }
