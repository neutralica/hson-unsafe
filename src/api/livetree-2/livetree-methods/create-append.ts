...
import { makeTreeSelector, TreeSelector } from "../tree-selector";
 ...
 /* overloads  */
export function createAppend2(this: LiveTree2, tag: TagName, index?: number): LiveTree2;
export function createAppend2(this: LiveTree2, tags: TagName[], index?: number): TreeSelector;
/*  implementation */
export function createAppend2(
  this: LiveTree2,
  tagOrTags: TagName | TagName[],
  index?: number,
): LiveTree2 | TreeSelector {
  const tags: TagName[] = Array.isArray(tagOrTags) ? tagOrTags : [tagOrTags];

  // ensure there *is* an anchor node; throws if tree is unbound
  void this.node;

  const created: LiveTree2[] = [];
  ...
  for (const t of tags) {
    ...
    for (const child of appended) {
      const childTree = new LiveTree2(child);
