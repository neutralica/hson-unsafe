...
function emitNode(
    ...
    guard: ReturnType<typeof cycleGuard>       /* <-- NEW */
): string {
    ...
    try {
        ...
        guard.leave(node);                               /* <-- NEW */
    }
}

/* public API (NEW) */
export function serialize_hson_NEW(root: HsonNode_NEW): string {
    _log('beginning HSON serialization');
    if (!(root && typeof root === "object" && "_tag" in root)) {
        _throw_transform_err("serialize-hson: root must be a HsonNode_NEW", 'serialize-hson');
    }
    const guard = cycleGuard();                    // <-- NEW
    const out = emitNode(root, 0, undefined, guard);  // <-- pass guard
    return out.trim();
}